...
my_reduced_Granges <- test1 %>% 
  group_by(revmap) %>% 
  reduce_ranges_directed
my_median <- test1 %>% select(C_Ab_1_peakExpr, revmap) %>% as_tibble() %>% group_by(revmap) %>% 
...
result_med_sliced <- my_1st_med %>% filter(seqnames == "chr1") %>% as_tibble() %>% select(revmap,C_Ab_1_peakExpr)
