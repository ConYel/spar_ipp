...
identical(t2$start,t3$start)


samples_granges <- gr1$samples_file %>% enframe %>%  distinct(value) %>% arrange(value) 
l_samples_granges <- samples_granges$value %>% as.list()

grSamples <- lapply(l_samples_granges, function(x){
  gr1  %>% filter(samples_file == x) %>% 
  arrange(start)
  names()}
)

C_Ab_1 <- gr1 %>% filter(samples_file == "C_Ab_1") %>% 
  select(C_Ab_1_counts = "peakExpressionValue",everything()) %>% arrange(start)
C_Ab_2 <- gr1 %>% filter(samples_file == "C_Ab_2") %>% 
  select(C_Ab_2_counts = "peakExpressionValue",everything())%>% arrange(start)
C_Ab_3 <- gr1 %>% filter(samples_file == "C_Ab_3") %>% 
  select(C_Ab_3_counts = "peakExpressionValue",everything())%>% arrange(start)


t3 %>% join_overlap_left_directed( C_Ab_1) %>% 
  mutate(C_Ab_1_counts = replace_na(C_Ab_1_counts,0)) %>% 
  select(-samples_file) %>% 
  join_overlap_left_directed( C_Ab_2) %>% 
  mutate(C_Ab_2_counts = replace_na(C_Ab_2_counts,0)) %>% 
  select(-samples_file) %>% 
  join_overlap_left_directed( C_Ab_3) %>% 
  mutate(C_Ab_3_counts = replace_na(C_Ab_3_counts,0)) %>% 
  select(-samples_file) 
```
