...
features_NOIS <- map(NOISeq_filt, ~ .x %>% 
                       rownames() %>%
                       enframe(name = NULL)) 
features_NOIS <- list(features_NOIS[[1]],features_NOIS[[3]])

features_edgeR <- map(list(dgl_filt, dgl_filt_2) , ~ .x %>% 
                       rownames() %>%
                       enframe(name = NULL)) %>% set_names("batch", "no_batch")

common_edgeR_batch <-  map2(features_edgeR[1] , features_NOIS, ~ .x %>% inner_join(.y))
common_edgeR_nobatch <-  map2(features_edgeR[2] , features_NOIS, ~ .x %>% inner_join(.y))
...
piRbase_olaps <- DE_ranges_DESQ %>% join_overlap_left_directed(pirbase, minoverlap = 5) %>% 
  filter(!is.na(name)) %>% 
  as_tibble() 
...
piRNA_IPP <- list.files("/home/0",full.names = TRUE, pattern = "piRNAs316.txt", recursive = TRUE) %>% 
