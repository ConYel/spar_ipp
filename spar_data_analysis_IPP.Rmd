...
# 1.1a make matrix for DE analysis  -----
...
# 1.1b load the not annot matrix for DE analysis  -----
dt <- fread("all_peaks_smRNAs.txt")
...
## in case there were SRA target files ----
...
# make the matrix  
mat <- dt %>% 
  unite("smallRNA",c(smallRNA:GeneClass), sep = "_") %>%
...
names(mybiotypes) <- rownames(mat)
# NOISeq dataset
mydata <- NOISeq::readData(data = mat, 
                           factors = as.data.frame(targets_file), 
                           biotype = mybiotypes)

mybiodetection <- NOISeq::dat(mydata, k = 0, type = "biodetection")
par(mfrow = c(1, 2))
explo.plot(mybiodetection, samples = c(1,4), toplot = "piRNA",plottype = "comparison")
...
NOISeq_filt <- sapply(noiL, function(x) NOISeq::filtered.data(mat, 
        factor = targets_file$group,
        norm = FALSE, 
        method = x, cv.cutoff=100, cpm=1), simplify = FALSE, USE.NAMES = TRUE)
...
## more strigent filter
design_2 <- model.matrix(~0 + targets_file$group)
colnames(design_2) <- colnames(design_2) %>% 
  str_remove("targets_file\\$") %>% 
  str_remove("group")
rownames(design_2) <- targets_file$Run
...
keep.exprs_2 <- filterByExpr.DGEList(dgl, design = design_2)
dgl_filt <- dgl[keep.exprs,,keep.lib.sizes=FALSE]
dgl_filt_2 <- dgl[keep.exprs_2,,keep.lib.sizes=FALSE]
dim(dgl);dim(dgl_filt);dim(dgl_filt_2)

 map(smpls_lst, ~ gr1  %>% 
      filter(samples_file == .x) %>% 
      select(!!str_c(.x, "_peakExpr") := "peakExpressionValue",everything()))
 
features_NOIS <- map(NOISeq_filt, ~ .x %>% 
                       rownames() %>%
                       enframe(name = NULL)) 

features_edgeR <- map(list(dgl_filt, dgl_filt_2) , ~ .x %>% 
                       rownames() %>%
                       enframe(name = NULL)) %>% set_names("batch", "no_batch")

common_edgeR_batch <-  map2( features_NOIS[1], features_edgeR, ~ .x %>% inner_join(.y))
common_edgeR_nobatch <-  map2( features_NOIS[3], features_edgeR, ~ .x %>% inner_join(.y))

str_glue( "Starting features: {nrow(dgl)}\nedgeR filter, remaining: {nrow(dgl_filt)}\nedgeR no batch, remaining: {nrow(dgl_filt_2)}\nCPM filter, remaining: {nrow(NOISeq_filt[[1]])}, common edgeR_batch: {nrow(common_edgeR_batch[[1]])}, common edgeR_no_batch:{nrow(common_edgeR_batch[[2]])}\nWilcoxon filter, remaining features: {nrow(NOISeq_filt[[2]])}\nProportion filter, remaining features: {nrow(NOISeq_filt[[3]])}, common edgeR_batch: {nrow(common_edgeR_nobatch[[1]])}, common edgeR_no_batch:{nrow(common_edgeR_nobatch[[2]])} ")

# if we want to keep noiseq filter instead of edger but use edger object ----
dgl_filt <- dgl[rownames(dgl$counts) %in% rownames(NOISeq_filt[[i]]),,keep.lib.sizes=FALSE]
## or
...
EDASeq::biasPlot(dataEDA, "gc", log=TRUE, ylim=c(1,2))
```
## histogram
```{r}
...
hist(DEseq2_res_N_Ab_vs_Control$pvalue[DEseq2_res_N_Ab_vs_Control$baseMean > 1], 
     breaks = 0:20/20,
     col = "grey50", border = "white")
hist(DEseq2_res_C_Ab_vs_Control$pvalue[DEseq2_res_C_Ab_vs_Control$baseMean > 1], 
     breaks = 0:20/20,
     col = "grey50", border = "white")
hist(DEseq2_res_INPUT_vs_Control$pvalue[DEseq2_res_INPUT_vs_Control$baseMean > 1], 
     breaks = 0:20/20,
     col = "grey50", border = "white")
...
par(mfrow=c(1,2))
edger_norm %>% names()
plotMD(edger_norm[[2]],column = 3)
plotMD(edger_norm[[2]],column = 4)
...
topedgeR_res_INPUT_vs_Ctl <- topTags(edgeR_res_INPUT_vs_Ctl, 
                            n = nrow(edgeR_res_INPUT_vs_Ctl), 
                            adjust.method = "BH", 
                            sort.by = "PValue", 
                            p.value = 1)$table %>% 
  as_tibble(rownames = "smallRNA") %>% 
  write_tsv(str_glue("{dat_path}/edgeR_res_INPUT_vs_Ctl_{todate}_{my_exp}.txt"))

hist(topedgeR_res_INPUT_vs_Ctl$PValue, breaks = 0:20/20,
     col = "grey50", border = "white")
### C ab
edgeR_res_C_vs_Ctl <- glmQLFTest(fit,contrast = con_mat[,"C_vs_Ctl"])
is.de <- decideTests(edgeR_res_C_vs_Ctl)
summary(is.de)
topTags(edgeR_res_C_vs_Ctl,n=10)$table 
plotMD(edgeR_res_C_vs_Ctl,
       status = is.de,
       values = c(1,-1),
       col = c("red","blue"),
       legend="topright")

topedgeR_res_C_vs_Ctl <- topTags(edgeR_res_C_vs_Ctl, 
                            n = nrow(edgeR_res_C_vs_Ctl), 
                            adjust.method = "BH", 
                            sort.by = "PValue", 
                            p.value = 1)$table %>% 
  as_tibble(rownames = "smallRNA") %>% 
  write_tsv(str_glue("{dat_path}/edgeR_res_C_vs_Ctl_{todate}_{my_exp}.txt"))

hist(topedgeR_res_C_vs_Ctl$PValue, breaks = 0:20/20,
     col = "grey50", border = "white")

### N ab
edgeR_res_N_vs_Ctl <- glmQLFTest(fit,contrast = con_mat[,"N_vs_Ctl"])
is.de <- decideTests(edgeR_res_N_vs_Ctl)
summary(is.de)
topTags(edgeR_res_N_vs_Ctl,n=10)$table 
plotMD(edgeR_res_N_vs_Ctl,
       status = is.de,
       values = c(1,-1),
       col = c("red","blue"),
       legend="topright")

topedgeR_res_N_vs_Ctl <- topTags(edgeR_res_N_vs_Ctl, 
                            n = nrow(edgeR_res_N_vs_Ctl), 
                            adjust.method = "BH", 
                            sort.by = "PValue", 
                            p.value = 1)$table %>% 
  as_tibble(rownames = "smallRNA") %>% 
...
hist(topedgeR_res_N_vs_Ctl$PValue, breaks = 0:20/20,
     col = "grey50", border = "white")
### edgeR venny
...
hist(voom_res_INPUT_vs_Ctl$P.Value, 
     breaks = 0:20/20,
     col = "grey50", border = "white")
hist(voom_res_N_vs_Ctl$P.Value, 
     breaks = 0:20/20,
     col = "grey50", border = "white")
hist(voom_res_C_vs_Ctl$P.Value, 
     breaks = 0:20/20,
     col = "grey50", border = "white")
...
voom_IPP <- C_IP %>% 
  ...
  write_tsv(str_glue("{dat_path}/voomQW_TMM_res_common_{todate}_{my_exp}.txt"))

voom_IPP_union <- C_IP %>% 
  full_join(N_IP) %>%
  filter(!smallRNA %in% INPUT_IP$smallRNA) %>% 
  select(smallRNA, C_logFC, C_adj.P.Val, N_logFC, N_adj.P.Val) %>% 
  write_tsv(str_glue("{dat_path}/voomQW_TMM_res_union_{todate}_{my_exp}.txt"))
## DEseq2 ---
...
C_D2 %>% inner_join(N_D2)  %>% anti_join(INPUT_D2) %>% select(GeneID)
DEseq_IPP <- C_D2 %>% 
  inner_join(N_D2) %>%
  filter(!GeneID %in% INPUT_D2$GeneID) %>% 
  select(GeneID, C_log2FoldChange, C_padj, N_log2FoldChange, N_padj) %>% 
  write_tsv(str_glue("{dat_path}/DESEQ_res_common_{todate}_{my_exp}.txt"))

DEseq_IPP_union <- C_D2 %>% 
  full_join(N_D2) %>%
  filter(!GeneID %in% INPUT_D2$GeneID) %>% 
  select(GeneID, C_log2FoldChange, C_padj, N_log2FoldChange, N_padj) %>% 
  write_tsv(str_glue("{dat_path}/DESEQ_res_union_{todate}_{my_exp}.txt"))

DEseq_IPP %>% filter(GeneID %in% voom_IPP$smallRNA)
DEseq_IPP_union %>% filter(GeneID %in% voom_IPP_union$smallRNA)
```
## Annotate the peaks
```{r}
library(plyranges)
DE_ranges_DESQ <- DEseq_IPP_union %>%  
  separate(GeneID, c("seqnames", "start", "end", "width", "strand", "smallRNA","DQ"),sep = ":", convert = TRUE) %>% 
   separate(DQ, c("DQ","GeneClass"), sep = "_") %>% 
  plyranges::as_granges()

DE_ranges_voom <- voom_IPP_union %>%  
  separate(smallRNA, c("seqnames", "start", "end", "width", "strand", "smallRNA","DQ"),sep = ":", convert = TRUE) %>% 
   separate(DQ, c("DQ","GeneClass"), sep = "_") %>% 
  plyranges::as_granges()
...
annotated_peaks <- matchGenes(DE_ranges, genes, type = "any", promoterDist = 2500, skipExons = FALSE, verbose = TRUE) %>% as_tibble()

bed_file <- list.files("/home/0",full.names = TRUE, pattern = "piR.+.bed", recursive = TRUE)

library(plyranges)
pirbase <- read_bed(bed_file)

piRbase_olaps <- DE_ranges_voom %>% 
  join_overlap_left_directed(pirbase, minoverlap = 5) %>% 
  filter(!is.na(name)) %>% 
  as_tibble() 

piRBnas <- pirbase %>% 
  filter(name %in% piRbase_olaps$name) %>% 
  as_tibble() %>% 
  rename_at(vars(-c("seqnames", "name","score")), function(.) str_c("piRBase_", .))

piRbase_olaps %>% 
  inner_join(piRBnas) %>% 
  write_tsv(str_glue("{dat_path}/piRBase_DE_DESEQ2_common_{todate}_{my_exp}.txt"))

piRNA_IPP <- list.files("/home/0",full.names = TRUE, pattern = "piRNAs316.txt",
                        recursive = TRUE) %>% 
  read_tsv(col_names = FALSE) %>% 
  separate(X1, c("class", "piRNA"), sep = "_match_")

piRbase_olaps %>% filter(name %in% piRNA_IPP$piRNA)

pirbase_red <- pirbase %>% 
  filter(name %in% piRNA_IPP$piRNA) %>% 
  as_tibble()
# check all dataset-----
complete_dat <- voom_res_C_vs_Ctl  %>%  
  separate(smallRNA, c("seqnames", "start", "end", "width", "strand", "smallRNA","DQ"),sep = ":", convert = TRUE) %>% 
   separate(DQ, c("DQ","GeneClass"), sep = "_") %>% 
  plyranges::as_granges()

piR_316 <- complete_dat %>% 
  join_overlap_left_directed(pirbase_red, minoverlap = 5) %>% 
  filter(!is.na(name)) %>% 
  as_tibble() 

## make tracks from SPORTS and SPAR workflow ----
library(tidyverse)
library(plyranges)
#load the files
bed_file <- list.files("/home/0",full.names = TRUE, pattern = "piR.+.bed", recursive = TRUE)
piRNA_IPP <- list.files("/home/0",full.names = TRUE, pattern = "piRNAs316.txt",
  ...
  read_tsv(col_names = FALSE) %>% 
  separate(X1, c("class", "piRNA"), sep = "_match_")
BAM_IPP <- list.files("/home/0",full.names = TRUE, pattern = "1_sorted.bam",
                        recursive = TRUE) %>% 
  read_bam()
BAM_IPP <- rbamtools::bamReader(list.files("/home/0",full.names = TRUE, pattern = "1_sorted.bam",
                        recursive = TRUE), indexname = list.files("/home/0",full.names = TRUE, pattern = "1_sorted.bai",
                        recursive = TRUE))
rbamtools::createIndex(BAM_IPP, idx_filename="IPP_C_ab_1_sorted.bai")

BAM_IPP <- list.files("/home/0",full.names = TRUE, pattern = "sorted.bed", recursive = TRUE) %>% read_bed
pirbase <- read_bed(bed_file)
pirbase316 <- pirbase %>% filter(name %in% piRNA_IPP$piRNA)

sInfo <- Seqinfo(genome="hg38")
seqlevels(sInfo) <- seqlevels(pirbase316)
seqinfo(pirbase316)<- sInfo
pirbase316 %>% write_bed("piRBase316.bed")
write_bigwig("piRBase316.bigWig")

peaks <- read_tsv(list.files("/home/0",full.names = TRUE, pattern = "peaks_all.xls",
                recursive = TRUE)) %>% 
  dplyr::rename(peakChr = "#peakChr") %>%  
  select(peakChr:peakStrand) %>% 

  as_granges(seqnames = peakChr,
                         start = peakChrStart,
                         end = peakChrEnd,
                         strand = peakStrand) %>% 
  arrange(start) %>% write_bed("peaks_all.bed")
...
num_1 <- test2 %>% select(C_Ab_1_peakExpr, C_Ab_2_peakExpr, C_Ab_3_peakExpr)
num_2 <- my_unAnnot_GR %>% select(C_Ab_1_peakExpr, C_Ab_2_peakExpr, C_Ab_3_peakExpr)
identical(num_1, num_2)
identical(as_tibble(num_1), as_tibble(num_2))
#########################################################################
